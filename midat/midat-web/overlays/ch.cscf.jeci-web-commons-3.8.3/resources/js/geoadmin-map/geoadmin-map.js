    ...
    return {
        ...
        link : function(scope, element){
            ...
            if(scope.model.rectangleSelector){
                ...
                scope.changeMapFont= function(){

                    var elem =$('#changeFontBtnId');
                    var layer=null;
                    if( elem.hasClass("fa-map-o")){

                        elem.removeClass("fa-map-o");
                        elem.addClass("fa-map");
                        elem.addClass("coloerd-map");
                        layer = ga.layer.create('ch.swisstopo.pixelkarte-grau');
                    }else{
                        elem.removeClass("fa-map");
                        elem.addClass("fa-map-o");
                        elem.removeClass("coloerd-map");
                        layer = ga.layer.create('ch.swisstopo.pixelkarte-farbe');
                    }
                    var layersCollection =    map.getLayers();
                    if(layersCollection.getArray().length >0){
                        layersCollection.removeAt(0);
                    }

                    var baseLayerGroup = new ol.layer.Group({layers : [layer]});
                    layersCollection.insertAt(0, baseLayerGroup);
...
module.factory("olStylesFactory", [function(){
    ...
    var featurePointStyleSmall = new ol.style.Circle({
        radius: 6,
        fill: new ol.style.Fill({color: "#044E7D"}),
    ...
    return {
        ...
        featureV2TextStyle : function(text,color){
            if(!featureTextStyleCache[text]){
                featureTextStyleCache[text] = new ol.style.Text({
                    font: 'bold 16px Helvetica,Arial,sans-serif',
                    text: text,
        ...
        featureV2TextStyleSelected : function(text,color){
            if(!featureTextStyleSelectedCache[text]){
                featureTextStyleSelectedCache[text] = new ol.style.Text({
        ...
        featureV2PointStyleSmall : function(color){
            var featureV2PointStyleSmall = new ol.style.Circle({
                radius: 6,
                fill: new ol.style.Fill({color: color}),
                stroke: new ol.style.Stroke({color: "white", width: 3})
            });
            return featureV2PointStyleSmall;
        },

        featureV2PointStyleSmallSelected : function(color){
            var featureV2PointStyleSmallSelected = new ol.style.Circle({
                radius: 6,
                fill: new ol.style.Fill({color: color}),
                stroke: new ol.style.Stroke({color: "#0AFFF3", width: 3})
            });
            return featureV2PointStyleSmallSelected;
        },

        featureV2PointStyleBig : function(color){
            var featureV2PointStyleBig = new ol.style.Circle({
                radius: 10,
                fill: new ol.style.Fill({color: color}),
                stroke: new ol.style.Stroke({color: "#0AFFF3", width: 4})
            });
            return featureV2PointStyleBig;
        },

        featureV2PointStyleBigSelected : function(color){
            var featureV2PointStyleBigSelected = new ol.style.Circle({
                radius: 10,
                fill: new ol.style.Fill({color: color}),
                stroke: new ol.style.Stroke({color: "#0AFFF3", width: 4})
            });
            return featureV2PointStyleBigSelected;
        }
